...
export class MediaHelper {
  ...
  private urlIntoSandbox(srcPath: string, dstPath: string): string {
    ...
    Logger.info(this.TAG, 'filePath: ', srcPath)
      ...
      return photoPicker.select(photoSelectOptions)
        .then((photoSelectResult: photoAccessHelper.PhotoSelectResult) => {
          ...
          if (photoSelectResult && photoSelectResult.photoUris && photoSelectResult.photoUris.length > 0) {
            ...
            Logger.info(this.TAG, 'mContext: ', this.mContext.filesDir)
            ...
            Logger.info(this.TAG, '沙箱中文件路径(before copy): ', path)
            ...
            Logger.info(this.TAG, '沙箱中文件路径(after copy): ', this.uri)
  ...
  public readFileAsUint8Array(): Promise<Uint8Array> {
    try {
      ...
      Logger.info('Base 64', 'opened path:', openedFile.path)
      Logger.info('Base 64', 'pathDir:', this.mContext.filesDir)
      Logger.info('Base 64', 'File descriptor:', openedFile.fd)

      // todo: 问题所在之处， 无法将文件数据读取到缓存中

      fs.readSync(openedFile.fd, buffer, { offset: 0, length: stats.size })

      // Logger.info('Base 64', 'File allocated buffer after read:', StringUtil.arrayBuffer2String(buffer))
      ...
      return new Promise<Uint8Array>((resolve) => {
        ...
        Logger.info('Base 64', 'File successfully read as Uint8Array:', uint8Array);
        // unit8ArrayPrint()
        resolve(uint8Array)
      })
    } catch (err) {
      Logger.error('Base 64', 'Error reading file:', err);
      Logger.error('Base 64', 'Error reading file:', this.mContext);
      return Promise.reject(err);
    }
  }
  ...
  public async convertToBase64() {
    try {
      ...
      Logger.error(this.TAG, 'Base 64', 'Error converting to Base64:', err);
  ...
  public async takePhoto(context: common.UIAbilityContext): Promise<MediaBean> {
    ...
    return context.startAbilityForResult(want)
      .then((result) => {
        Logger.info(this.TAG, `startAbility call back , ${JSON.stringify(result)}`);
        Logger.info(this.TAG, `startAbility call back outside ${(result.want.parameters['resourceUri'])} ${StringUtil.isNotNullOrEmpty(result.want.uri)}`);
        let photoUrl: string = (result.want.parameters['resourceUri']).toString()
        if (result.resultCode === 0 && result.want && StringUtil.isNotNullOrEmpty(photoUrl)) {
          //拍照成功
          Logger.info(this.TAG, `startAbility call back inside`);
          // Logger.info(this.TAG, 'takePhoto successfully, takePhotoResult uri: ' + result.want.uri);
        ...
        Logger.info(this.TAG, `startAbility error , ${JSON.stringify(error)}`);
        return error;
  ...
  private async appendFileInfoToMediaBean(mediaBean: MediaBean, uri: string) {
    ...
    if (fileList && fileList.length > 0) {
      ...
      // Logger.info(this.TAG, `file id = ${JSON.stringify(fileInfoObj.id)} , uri = ${JSON.stringify(fileInfoObj.uri)}`);
      // Logger.info(this.TAG, `file fileList displayName = ${fileInfoObj.displayName} ,size = ${fileInfoObj.size} ,mimeType = ${fileInfoObj.mimeType}`);
